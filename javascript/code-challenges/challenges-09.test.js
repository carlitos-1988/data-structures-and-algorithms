...
const maxInArray = (arr) => {
  // Solution code here...
  let myVaue = -1;
  arr.reduce((previousVal,currentVal)=> currentVal>previousVal ? myVaue=currentVal: myVaue=previousVal);
  return myVaue;
};
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  houses = arr.map((person)=> person.house);
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  //let mySearch = Object.values(arr).map(x =>x.children).flat().includes(character);
  //return mySearch;
